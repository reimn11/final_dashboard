...
**Data source:** *Sleep Health and Lifestyle Dataset* (synthetic data), created by the author of the Kaggle dataset and made available on Kaggle:\
...
# Compute the correlation AND label positions per sleep_measure
cor_sleep <- sleep_long %>%
  group_by(sleep_measure) %>%
  summarise(
    correlation = cor(`Stress Level`, value, use = "complete.obs"),
    x = max(`Stress Level`, na.rm = TRUE) - 0.5, 
    y = max(value, na.rm = TRUE) - 0.1,         
  )
...
p_stress_sleep <- sleep_long %>%
  ...
  geom_text(
    data = cor_sleep,
    inherit.aes = FALSE,
    aes(x = x, y = y, label = paste0("r = ", round(correlation, 2))),
    size  = 4) +
...
# Correlation between Physical Activity and each sleep measure
cor_pa_sleep <- sleep_pa_long %>%
  group_by(sleep_measure) %>%
  summarise(
    correlation = cor(`Physical Activity Level`, value, use = "complete.obs"),
    x = max(`Physical Activity Level`, na.rm = TRUE) - 3,  
    y = max(value, na.rm = TRUE) + 1,                   
  )
...
sleep_pa_long %>% 
  ...
  labs(
    title = "Relationship between Physical Activity level and sleep duration/quality",
    x = "Physical Activity Level (min)",
    y = "Value"
  ) +
  theme_minimal()
